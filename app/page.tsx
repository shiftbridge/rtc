...
function HomeEntryForm() {
  ...
  useEffect(() => {
    ...
    if (error === "taken") {
      setErrorMsg(
        "That username is already active in that room. Please choose another one!",
      );
    }
  }, [searchParams]);
  ...
  const handleJoin = (e: FormEvent<HTMLFormElement>) => {
    ...
    router.push(
      `/room/${room.toLowerCase().trim()}?username=${encodeURIComponent(username.trim())}`,
    );
  };
      ...
      <div className="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-8 shadow-xl shadow-slate-200/50">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Enter your Username and Room Name and chat
          </p>
        </div>
